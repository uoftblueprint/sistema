...
import { isTokenValid, refreshAccessToken } from './src/services/login/AuthService';
...
axios.interceptors.request.use(async (config) => {
  ...
  config.headers.Authorization = `${store.getState().auth.driveToken.type} ${store.getState().auth.driveToken.token}`;
  return config;
}, (error) => {
